...
| | |
|---|---|
| **Mitigation** | The chat prompt reaches an **external cloud provider** only when the user explicitly selects a BYOK cloud provider (`openai` / `openrouter`, their own key). With local providers such as **OpenVINO/OVMS** (or Ollama / LM Studio), prompts remain within the local machine or the school network — a local OVMS request is **not** external-cloud egress. With **Mock**, nothing leaves the device. The UI shows an explicit warning before any cloud provider is used, in onboarding, Settings and the Model Lab. Every run mode, the deployed site included, starts on **Mock**. On the public deployment, managed chat is disabled, so no prompt reaches an external chat model; **managed embeddings send only the retrieval query text to Workers AI** — the student's question in Practice, and the task prompt plus the student's answer when retrieving grounding for grading (the chunk vectors are already in the downloaded pack). The proxy keeps any keys server-side, enforces a model allowlist, caps the request body and output tokens, times out slow upstreams, rejects cross-origin calls, and logs no secret / IP / prompt / answer. **The 2026 field deployment used a cloud LLM through the application's API-backed cloud path (hosted PWA over the Internet + cloud LLM), so that cohort's prompts did leave the school network; the local OpenVINO option was validated separately and was not used with that cohort** — see [FIELD_DEPLOYMENT.md](./FIELD_DEPLOYMENT.md). That was the configuration at the time; the current public demo does not use managed cloud chat. |
