...
async function main(): Promise<void> {
  ...
  const payload = {
    ...
    note:
      'Characterization of the reproducible deterministic public-fallback path, which uses a ' +
      'lightweight hash-embedding stub. DEFAULT_MIN_SIMILARITY (0.42) is calibrated for bge-m3, ' +
      'not the stub, so Subset A numbers are a fallback-boundary characterization — NOT a ' +
      'production semantic-refusal score. Subset A cases carry human-authored shouldRefuse labels; ' +
      'mismatches are recorded findings, not failures. The production bge-m3 refusal evaluation is ' +
      'tracked separately (P1-1b). Report only, no pass/fail gate. See docs/EVALUATION.md.',
    config: {
      topK: DEFAULT_TOP_K,
    ...
    citation: {
      ...
      // The mechanically meaningful evidence: an adversarial marker set and
      // whether the checker stripped every fabricated one.
      markerCounts: citation.markerCounts,
      // Per-fixture mean ratios. Value depends on fixture composition (how many
      // fabricated markers we authored) — diagnostic only, NOT a system score.
      fixtureCompositionDiagnostics: {
  ...
  const mc = citation.markerCounts
  console.log(
    `[safety] citation: fixtures=${citation.fixtureCount} exactMatch=${citation.passCount} — ` +
      `${mc.totalRawMarkers} raw markers, ${mc.rawFabricated} deliberately fabricated, ` +
      `${mc.fabricatedCaught}/${mc.rawFabricated} caught, ` +
      `${mc.invalidRemainingAfterSanitization} invalid remaining after sanitization`,
  )
  console.log(
    `[safety] citation: rawCitationValidityMean=${citation.rawCitationValidityMean.toFixed(3)} ` +
      `(fixture-composition diagnostic — depends on how many fabricated markers we authored, not a system score)`,
  )
  for (const r of citation.results.filter((x) => !x.pass))
    console.log(`[safety]   FIXTURE MISMATCH ${r.id}: ${r.mismatches.join(' | ')}`)
  console.log(
    `[safety] golden over-refusal signal (deterministic stub path, NOT a production figure): ` +
      `evaluated=[${goldenSignal.evaluatedSubjects.join(', ')}] ` +
